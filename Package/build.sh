	...
	case ${OPENWRT_SOURCE_VER} in
		...
		23)
			if [ "$ARCH" = "bcm2708" ] ; then
				...
				rootfs_url="https://archive.openwrt.org/releases/23.05.4/targets/x86/64/openwrt-23.05.4-x86-64-rootfs.tar.gz"
				version="https://archive.openwrt.org/releases/23.05.4/targets/x86/64/version.buildinfo"
			else
				echo "Unsupported architecture!"
				exit 1
			fi;;
			...
			24)
  			if [ "$ARCH" = "bcm2708" ] ; then
  				...
  				rootfs_url="https://archive.openwrt.org/releases/24.10.0/targets/x86/64/openwrt-24.10.0-x86-64-rootfs.tar.gz"
  				version="https://archive.openwrt.org/releases/24.10.0/targets/x86/64/version.buildinfo"
  			else
  				echo "Unsupported architecture!"
  				exit 1
  			fi;;
		*)
			if [ "$ARCH" = "bcm2708" ] ; then
				img_url="htt ps://downloads.openwrt.org/releases/${OPENWRT_SOURCE_VER}/targets/brcm2708/bcm2708/openwrt-${OPENWRT_SOURCE_VER}-brcm2708-bcm2708-rpi-squashfs-factory.img.gz"
				version="https://downloads.openwrt.org/releases/${OPENWRT_SOURCE_VER}/targets/brcm2708/bcm2708/version.buildinfo"
				gen_rootfs_from_img
				return
			elif [ "$ARCH" = "armvirt-32" ] ; then
